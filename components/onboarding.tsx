...
// ── Screen 3 ──────────────────────────────────────────────────
function AuthButton({ children, bg, color, border, icon, shadow, onClick, disabled }: {
  children: React.ReactNode; bg: string; color: string;
  border?: string; icon: React.ReactNode; shadow?: string; onClick: () => void; disabled?: boolean;
}) {
  return (
...
function Screen3({ onDone }: { onDone: () => void }) {
  ...
  const handleGoogle = async () => {
    ...
    // callbackURL "/" → page.tsx akan cek session dan skip onboarding
    setLoading(false);
  };
    ...
    <div style={{ width: "100%", height: "100%", display: "flex", flexDirection: "column" }}>
      ...
      <div style={{ padding: "0 28px", textAlign: "center" }}>
        ...
        <p style={{ fontFamily: "var(--font-jakarta)", fontSize: 15, lineHeight: 1.5, color: C.muted, margin: 0, maxWidth: 300, marginLeft: "auto", marginRight: "auto" }}>
          Agar perjalananmu lebih tenang
        </p>
      </div>
      ...
      <div style={{ padding: "0 24px", display: "flex", flexDirection: "column", gap: 14 }}>
        <AuthButton
          ...
          icon={
            loading ? (
              <div style={{ width: 22, height: 22, borderRadius: "50%", border: "2px solid #E8E3D6", borderTopColor: C.primary }} className="animate-spin-slow" />
            ) : (
        ...
        {/* WhatsApp — hidden until integration ready */}
      </div>
